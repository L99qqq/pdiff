...
# add by me
class AE_CNN_bottleneck_big(nn.Module):
    def __init__(self, in_dim, in_channel, time_step=1000, dec=None):
        super().__init__()

        # self.channel_list = [64, 128, 256, 1024]  # todo: self.channel_list*2
        self.channel_list = [64, 128, 256, 1024]  # todo: self.channel_list*2
        ...
        self.enc1 = nn.Sequential(
            nn.InstanceNorm1d(self.real_input_dim),
            nn.Conv1d(
                in_channel, self.channel_list[0], self.kernal_size, stride=1, padding=1
            ),
            nn.LeakyReLU(),
            nn.InstanceNorm1d(self.real_input_dim),
            nn.Conv1d(
        ...
        self.dec4 = nn.Sequential(
            ...
            nn.InstanceNorm1d(self.real_input_dim // self.fold_rate),
            nn.Conv1d(
                self.channel_list[0], in_channel, self.kernal_size, stride=1, padding=1
            ),
        )

        self.time_encode = nn.Embedding(time_step, self.real_input_dim)

    def forward(self, input, time, cond):
        assert input.shape[1] * input.shape[2] == self.in_dim
        ...
        cond1=cond1.reshape(cond1.shape[0],1,-1)
        cond2=cond2.reshape(cond2.shape[0],1,-1)
        # print("\n AE forward: cond1 shape",cond1.shape," cond2 shape:",cond2.shape," time_info shape:",time_info.shape)
        time_info += cond1
        time_info += cond2
