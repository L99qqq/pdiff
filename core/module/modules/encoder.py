...
class ODEncoder(nn.Module):
    ...
    def build_layer(self, in_dim, kernel_size, fold_rate, input_channel, output_channel, last=False, if_start=False):
        ...
        layer = nn.Sequential(
            ...
            nn.Conv1d(input_channel if not if_start else 1, input_channel, kernel_size, stride=1, padding=2),
            nn.LeakyReLU(),
            nn.InstanceNorm1d(in_dim),
            nn.Conv1d(input_channel, output_channel, kernel_size, stride=fold_rate, padding=1),
...
# ae for 复现ConvNet3
# class big(ODEncoder2Decoder):
#     def __init__(self, in_dim, input_noise_factor, latent_noise_factor):
#         fold_rate = 5
#         kernel_size = 5
#         enc_channel_list = [32, 32, 32, 32]  # 编码器通道列表
#         dec_channel_list = [32, 512, 512, 32]  # 解码器通道列表
...
# ae for unet_attention time1
class big_time1(ODEncoder2Decoder):
    def __init__(self, in_dim, input_noise_factor, latent_noise_factor):
        fold_rate = 4
        kernel_size = 4
        enc_channel_list = [16,16,16,16]  # 编码器通道列表
        dec_channel_list = [16, 512, 512, 16]  # 解码器通道列表
        super(big, self).__init__(
            in_dim,
            kernel_size,
            fold_rate,
...
# ae for unet_attention time3
class big(ODEncoder2Decoder):
    def __init__(self, in_dim, input_noise_factor, latent_noise_factor):
        fold_rate = 5
        kernel_size = 3
        enc_channel_list = [112,112,112,112]  # 编码器通道列表
        dec_channel_list = [112,256,256,112]  # 解码器通道列表
        super(big, self).__init__(
            in_dim,
            kernel_size,
            fold_rate,
...
class big_cond2(ODEncoder2Decoder):
    def __init__(self, in_dim, input_noise_factor, latent_noise_factor):
        fold_rate = 4
        kernel_size = 4
        enc_channel_list = [48,48,48,48]  # 编码器通道列表
        dec_channel_list = [48,256,256,48]  # 解码器通道列表
        super(big_cond2, self).__init__(
            in_dim,
            kernel_size,
            fold_rate,
            input_noise_factor,
...
if __name__ == '__main__':
    ...
    # model = medium(155185, 0.1, 0.01)
    model = big(51124, 0.001, 0.1)

    # 创建一个示例输入
    example_input = torch.randn(1, 51124)  # 生成一个大小为 (1, in_dim) 的随机张量

    # 计算模型的输出
    output = model.forward(example_input)  # 假设 forward 方法是计算模型的输出
