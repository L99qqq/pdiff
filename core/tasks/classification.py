...
class CFTask(BaseTask):
    ...
    def train_for_data(self):
        ...
        for i in range(0, random_seed_num):
            ...
            writer_epoch = SummaryWriter(log_dir=os.path.join(final_path, "logs/seed_{}/epochs".format(i)))
            while save_num < each_seed_save_num:
                ...
                writer_epoch.add_scalar("train_loss", train_loss, current_epoch)
                writer_epoch.add_scalar("train_acc", train_acc, current_epoch)
                writer_epoch.add_scalar("test_acc", test_acc, current_epoch)
                ...
                ) or (current_epoch > 150 and start_save_flag == False):
                    start_save_flag = True
                    print(abs(max(accs[-20:-1]) - min(accs[-20:-1])))
                    
                    writer_save = SummaryWriter(log_dir=os.path.join(final_path, "logs/seed_{}/start_save_epoch".format(i)))
                    
                    writer_save.add_scalar("train_loss", train_loss, current_epoch)
                    writer_save.add_scalar(
                        "train_acc", train_acc, current_epoch
                    )
                    writer_save.add_scalar("test_acc", test_acc, current_epoch)
